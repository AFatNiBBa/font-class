
import { ComponentProps } from "solid-js";
import { Icon } from "../../index";

/**
 * A component that renders the `rhombus` icon from the `duotone` section of Font Awesome 6 Pro
 * @see {@link https://fontawesome.com/icons/rhombus?s=duotone rhombus}
 * @preview ![rhombus](data:image/svg+xml;base64,|width=32|height=32)
 */
export default function Rhombus(props: ComponentProps<typeof Icon>) {
    return <>
        <Icon viewBox="0 0 448 512" {...props}>
            <path opacity={.4} d="M442.189 240.265C446.063 244.765 448 250.359 448 255.953C448 261.547 446.063 267.141 442.189 271.642L242.245 503.656C237.434 509.219 230.717 512 224 512S210.566 509.219 205.755 503.656L5.811 271.642C1.937 267.141 0 261.547 0 255.953C0 250.359 1.937 244.765 5.811 240.265L205.755 8.25C210.566 2.75 217.283 0 224 0S237.434 2.75 242.245 8.25L442.189 240.265Z " />
        </Icon>
    </>
}