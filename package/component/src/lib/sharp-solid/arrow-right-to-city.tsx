
import { Icon } from "../../index";

/**
 * A component that renders the `arrow-right-to-city` icon from the `sharp-solid` section of Font Awesome 6.6 Pro
 * @see {@link https://fontawesome.com/icons/arrow-right-to-city?s=sharp-solid arrow-right-to-city}
 * @preview ![arrow-right-to-city](https://corsproxy.io/?https://site-assets.fontawesome.com/releases/v6.6.0/svgs/sharp-solid/arrow-right-to-city.svg)
 */
const ArrowRightToCity: typeof Icon = x => (
    <Icon viewBox="0 0 640 512" {...x}>
        <path d="M288 0L480 0l0 192 40 0 0-72 0-24 48 0 0 24 0 72 72 0 0 320-160 0-96 0-96 0L288 0zm64 64l0 64 64 0 0-64-64 0zm64 96l-64 0 0 64 64 0 0-64zm-64 96l0 64 64 0 0-64-64 0zm224 0l-64 0 0 64 64 0 0-64zm-64 96l0 64 64 0 0-64-64 0zM150.6 153.4l80 80L253.3 256l-22.6 22.6-80 80L128 381.3 82.7 336l22.6-22.6L130.7 288 32 288 0 288l0-64 32 0 98.7 0-25.4-25.4L82.7 176 128 130.7l22.6 22.6z" />
    </Icon>
);

export default ArrowRightToCity;